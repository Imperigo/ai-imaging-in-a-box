...
| C10 | **`geom_iou` weist aus, wie viel davon der Szene gehört** | 🟩 **gebaut, am Gerät unbestätigt** | 2026-08-28 | `geometrie_qa._iou_ohne_boden`, `tiefenschaetzer.qa_gegen_soll` (`geom_iou_norm`), `abholer.befund_kurz`. Owner-Entscheid 28.08.: der Score bleibt unangetastet, der normierte Wert steht daneben. Anlass: Ein konstantes Bild bekommt **exakt** den Vordergrundanteil (0.1104 · 0.1729 · 0.5297, auf volle Gleitkommagenauigkeit). Bestätigung am Gerät: `auftraege/offen/auf-20260826-57.json` |
| C11 | **Die Kamera steht waagrecht — der Entscheid vom 23.08. gilt jetzt auch im Runner** | 🟩 **gebaut, am Gerät unbestätigt** | 2026-08-28 | `src/aiimaging/runners/blender_depth_stage.py`, `tests/test_kameramodus_vorgabe.py`. Fünf Tage lang stand jeder Lauf auf `gekippt`, weil der Runner die Bibliothek überstimmte. Gemessen: Blickziel 4,50 m statt 1,45 m bei einem Auge auf 1,45 m. Angesagt und zu bestätigen: `auftraege/offen/auf-20260828-65.json` |
| C12 | **Der Demolauf macht ein Bild auf 1,70 m Augenhöhe** | 🟩 **gebaut, am Gerät unbestätigt** | 2026-08-28 | `tools/homeworker.py` (`VORGABE_KAMERA`, `_KAMERA_PARAMS`), `tests/test_homeworker.py`. **Der Homeworker forderte gar keine Kamera an** — der Runner stellte darum seine Notkamera (50 mm, `weg: rueckfall`). Gemessen nach der Änderung: `weg: abgeleitet`, `sSE`, 35 mm, Auge 1,45 m bei Gelände −0,25 m, Blickachse waagrecht. Angesagt und zu bestätigen: `auftraege/offen/auf-20260828-65.json` (V0) |
| C13 | **Der Homeworker liest, für wen ein Auftrag ist** | 🟩 **gebaut, am Gerät unbestätigt** | 2026-08-28 | `tools/homeworker.py` (`EIGENER_WORKER`), `tests/test_homeworker.py`. Befund vom Gerät (`auf-20260828-64`, V1): Das `worker`-Feld wurde **nirgends gelesen** — neun offene Aufträge an `cloud`/`ui` wären grün-leer geschlossen worden, und ein geschriebenes Ergebnis heisst hier *beantwortet*. **Muss vor dem Takt aus `auf-20260826-59` installiert sein.** |
| C14 | **Drei Angaben, die gelesen und nie verwendet wurden** | 🟩 **gebaut, am Gerät unbestätigt** | 2026-08-28 | `tools/homeworker.py`, `tests/test_homeworker.py`. Befunde vom Gerät (`auf-20260828-66`): **`erzeugen_mit` wurde ignoriert** — `--hochbau` erreichte den Aufruf nie, `auf-65` hätte ein anderes Gebäude gemessen; **der Kamerablock fehlte in `messwerte`** — V0 war aus dem Ergebnis nicht beantwortbar; **`ARTEN` wurde nirgends geprüft** — eine unbekannte Art lief still als Multipass. Zu bestätigen: `auftraege/offen/auf-20260828-65.json` |
