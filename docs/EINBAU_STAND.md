...
| A7 | Feldnamen gegen den vorgesehenen Vorgänger prüfen | 🟩 **halb** | 2026-09-17 | **Nachtrag 17.09.2026, und er schliesst die Frage auf der Gegenseite:** KosmoOrbit hat geantwortet (`Imperigo/Architektur-Cosmos`, Zweig `claude/kosmo-orbit-v1-build-pzxkbj`, `erg-20260917-37`). Im Vertrag `render-result.ts` sind **alle** Zahlenmasse des QA-Blocks `nullable().optional()` — `null` und ein ganz fehlender Schlüssel kommen beide an —, und `GeometryStatus` trägt `measured / not_measured / not_applicable`. **Die dritte Antwort steht drüben im Vertrag**, und zwar seit dem 23.08.2026, dem Tag unserer Frage. Umgekehrt konnten wir ihr eigenes `unklar` zu `auf-20260827-64` schliessen: `kosmovis_query_qa_verdict` liest **flache** Feldnamen, nicht `render-result/v2` — belegt in unserem `gate.als_kosmovis_verdikt`. Bleibt **halb**, weil die Verdrahtung weiterhin nur rechnerisch belegt ist und nicht am laufenden Cockpit. |  `contracts.LANE_FIELDS` und `mcp_schemas.pruefe_verdrahtbarkeit` belegen die Verdrahtbarkeit **rechnerisch**; am laufenden Cockpit gemessen ist sie **weiterhin nicht**. **Der Beleg ist am 10.09.2026 richtiggestellt worden:** `auf-20260826-58` hat A7 nicht gemessen, sondern nur **entsperrt** — die HomeStation sagt es selbst: *«Diese Sitzung hat das nur ENTSPERRT, aber die eigentliche A7-Messung … nicht durchgeführt.»* Seit A4 stehen die Schemata am Endpunkt vollständig da, damit ist die Messung jetzt **möglich**. Neu beauftragt: `auftraege/offen/auf-20260910-101.json` |
...
| C15 | **`herkunft.py` kennt keine Kennung `kosmoorbit`** | 🟩 **erledigt** | 2026-09-11 | **belegt am Gerät:** `auf-20260902-73` (M1–M3), beendet **06.09.2026**. Die HomeStation hat **zwei echte KosmoOrbit-Dateien** gefunden (`asset.generator == 'KosmoOrbit V1'`, aus einer wirklichen Browsersitzung, nicht aus unserer eigenen Kette) und die Höhenachse **an der Datei gemessen**: nur die drei rohen Achsenspannen gelesen, ohne jede Up-Achsen-Annahme, und gegen einen Blender-Bericht **derselben** Szene gehalten, der vorher in eigenem Z-up-System entstand. Lauf 17: X 106,229 m, Y **30,437 m**, Z 59,260 m — die Y-Spanne trifft die unabhängig bekannte Gebäudehöhe auf drei Nachkommastellen exakt; Lauf 16 ebenso (31,068 m). *«Die Übereinstimmung ist die Messung, nicht eine Vermutung über den Exporter.»* Eingebaut in `src/aiimaging/herkunft.py` als Eintrag `KosmoOrbit`, `Y_UP`. **Dafür brauchte es eine vierte Sicherheitslage:** `BELEGT` heisst dort ausdrücklich *«folgt aus der Norm»* und damit immer `Z_UP` — ein Wächter hält das fest. Eine zweifach exakte Messung als `VERMUTET` zu führen wäre eine Untertreibung, als `BELEGT` eine Gleichsetzung von Messung und Norm. Neu: `GEMESSEN`, zählt in `fordere_up_axis` wie `BELEGT` (`SICHER`), bleibt aber als eigenes Wort sichtbar. *Eine Sicherheit gehört an ihre Quelle.* |
